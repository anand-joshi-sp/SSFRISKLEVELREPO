...
/**
 * POST /caep/send-status-change
 * Body: { stream_id, payload: { principal, current_status, previous_status?, reason?, event_timestamp?, sub_id? } }
 */
app.post("/caep/send-status-change", async (req, res) => {
  try {
    ...
    console.error("send-status-change error:", err.message);
    res.status(500).json({ error: "internal_error", message: err.message });
  }
});

/**
 * POST /caep/send-device-compliance-change
 * Body: { stream_id, payload: { sub_id, current_status, previous_status?, event_timestamp? } }
 */
